...
function extractSQLQuery(response: string): string | null {
    try {
...
function combineMessages(messages: { content: string; role: string }[]): string {
...
export async function generateSQL(messages: { content: string; role: string }[]) {
    try {
        console.log('HERE');
        ...
        return json({ error: "OpenAI request failed: " + errorMessage }, { status: 500 });
    }
}